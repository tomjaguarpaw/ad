...
efficiently calculate the gradients of a numerical program.  The
standard treatment uses mutation when the input program
which contain array indexing.  This note suggests an approach by
which reverse mode AD can be implemented efficiently on numerical
array programs in a pure language.
...
reverse mode AD on the following example program in a first-order
language
...
of fresh variables.  The form is similar to ANF from functional
...
has the type below (where $a$ ranges over numeric types) and which
in-place accumulates to an array at a given index, returning the array
after accumulation.  Recall that the input array to $\textrm{inc}$
...
In this note we present an approach which, we hypothesise, can
